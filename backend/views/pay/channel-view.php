...
<div class="main">
    ...
    <div class="databar">
    	...
    	<thead><tr><td>通道商</td><td>通道</td><td>标识</td><td>负责人</td><td>运营商</td><td>级别</td><td>DEV</td><td>短信</td><td>省份</td><td>时段</td><td>价目</td><td>接入</td><td>状态</td><td>日志</td><td>方式</td></tr></thead>
	...
	var succ	= function(resultJson){
		if(parseInt(resultJson.resultCode) == 1){
            ...
            $.each(resultJson.list,function(key,val){
                ...
                resultHtml = resultHtml + '<tr><td>'+val.merchant+'</td><td><a href="/pay/channel-detail-view?chid='+val.chid+'">'+val.name+'</a></td><td>'+val.sign+'</td><td>'+val.holder+'</td><td>'+val.provider+'</td><td>'+val.grade+'</td><td>'+val.devType+'</td><td><a class="glyphicon glyphicon-envelope" href="/pay/channel-verify-rule-view?chid='+val.chid+'"></a></td><td><a class="glyphicon glyphicon-globe" href="/pay/channel-regional-view?chid='+val.chid+'"></a></td><td><a class="glyphicon glyphicon-time" href="/pay/channel-time-limit-view?chid='+val.chid+'"></a></td><td><a class="glyphicon glyphicon-usd" href="/pay/channel-price-view?chid='+val.chid+'"></a></td><td><a data-devtypeid="'+val.devTypeId+'" data-chid="'+val.chid+'" class="glyphicon glyphicon-cog channel-config-entrance"></a></td><td class="'+statusClass+'"><a href="/pay/channel-status-view?chid='+val.chid+'">'+val.status+'</a></td><td><a class="glyphicon glyphicon-cog glyphicon-eye-open" href="/pay/channel-log-view?chid='+val.chid+'"></a></td><td><button class="cfgMainStatus btn" data-chid='+val.chid+'>'+cfgMainBtnName+'</button></td></tr>';
            });
