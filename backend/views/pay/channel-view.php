...
<div class="main">
    ...
    <div class="databar">
    	...
    	<thead><tr><td>通道商</td><td>通道</td><td>标识</td><td>负责人</td><td>运营商</td><td>DEV类型</td><td>状态</td><td>操作</td><td>配、手</td></tr></thead>
	...
	var succ	= function(resultJson){
		if(parseInt(resultJson.resultCode) == 1){
            ...
            $.each(resultJson.list,function(key,val){
                ...
                resultHtml = resultHtml + '<tr><td>'+val.merchant+'</td><td>'+val.name+'</td><td>'+val.sign+'</td><td>'+val.holder+'</td><td>'+val.provider+'</td><td><a class="chgDevType" data-chid="'+val.chid+'">'+val.devType+'</a></td><td>'+val.status+'</td><td><a data-devtypeid="'+val.devTypeId+'" data-chid="'+val.chid+'" class="glyphicon glyphicon-cog channel-config-entrance"></a></td><td><button class="cfgMainStatus btn" data-chid='+val.chid+'>'+cfgMainBtnName+'</button></td></tr>';
            });
